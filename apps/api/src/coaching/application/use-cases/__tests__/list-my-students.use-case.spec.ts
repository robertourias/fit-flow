...
describe("ListMyStudentsUseCase", () => {
  it("lists relationships where current user is the trainer", async () => {
    const relationshipRepo = makeRelationshipRepo();
    const usersRepo = makeUsersRepo();
    relationshipRepo.findByTrainer.mockResolvedValue([makeRelationship()]);

    const useCase = new ListMyStudentsUseCase(relationshipRepo, usersRepo);
    const result = await useCase.execute("trainer-1");

    expect(relationshipRepo.findByTrainer).toHaveBeenCalledWith("trainer-1", undefined);
  ...
  it("passes the status filter through to the repository", async () => {
    const relationshipRepo = makeRelationshipRepo();
    const usersRepo = makeUsersRepo();
    relationshipRepo.findByTrainer.mockResolvedValue([
      makeRelationship({ status: RelationshipStatus.PENDING }),
    ]);

    const useCase = new ListMyStudentsUseCase(relationshipRepo, usersRepo);
    await useCase.execute("trainer-1", RelationshipStatus.PENDING);

    expect(relationshipRepo.findByTrainer).toHaveBeenCalledWith(
      "trainer-1",
  ...
  it("includes REVOKED relationships when explicitly filtered", async () => {
    ...
    const useCase = new ListMyStudentsUseCase(relationshipRepo, usersRepo);
    const result = await useCase.execute("trainer-1", RelationshipStatus.REVOKED);
