...
@Injectable()
export class ListMyTrainersUseCase {
  constructor(
  ...
  async execute(studentId: string, status?: RelationshipStatus): Promise<RelationshipDto[]> {
    const relationships = await this._relationshipRepository.findByStudent(studentId, status);
